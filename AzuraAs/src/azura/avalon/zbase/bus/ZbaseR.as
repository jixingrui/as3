package azura.avalon.zbase.bus
{
	...
	public class ZbaseR extends PyramidFi implements BytesI,AstarMapI
	{
		public var shrinkZ:int;
		...
		public function fromBytes(zb:ZintBuffer):void
		{
			...
			for each(var fi:int in getPyramidIterator()){
				var tb:ZbaseTileR=getTile_(fi) as ZbaseTileR;
		...
		public function isRoad(x:int, y:int, accurate:Boolean=false):Boolean
		{
			x>>=shrinkZ;
			y>>=shrinkZ;
			var tile:ZbaseTileR = getTile(x, y, zMax) as ZbaseTileR;
			
			if (tile == null){
				//				trace("no such tile:",x,y,zMax-shrinkZ,this);
				return false;
			}
