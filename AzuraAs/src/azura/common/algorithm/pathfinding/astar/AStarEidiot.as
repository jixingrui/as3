package azura.common.algorithm.pathfinding.astar
{
	...
	public class AStarEidiot
	{
		 ...
		 */
		public function AStarEidiot(p_mapTileModel:AstarMapI, p_maxTry:int=1000)
		{
			this.m_map=p_mapTileModel;
		...
		private function check(p:Point,r:int):Point{
			if(m_map.isRoad(p.x,p.y))
				return p;
			
			for(var b:int=1;b<r;b++){
				for(var i:int=-b;i<=b;i++)
					for(var j:int=-b;j<=b;j++){
						if(i==-b||i==b||j==-b||j==b){
							if(m_map.isRoad(p.x+i,p.y+j)){
								trace(this,"found neighboring road");
		 ...
		 * @param p_endY		终点Y坐标
		 *
		 * @return 				null or 找到的路径(二维数组 : [p_startX, p_startY], ... , [p_endX, p_endY])
		 */
		public function find(start:Point, end:Point):Vector.<Point>
		{
			start=check(start,10);
			end=check(end,0);
			if(start==null||end==null)
				return null;
//			if(!m_map.isRoad(endX,endY))
//				return null;
			
			var p_startX:int=end.x;
			var p_startY:int=end.y;
			var p_endX:int=start.x;
			var p_endY:int=start.y;
			this.initLists();
			this.m_openCount=0;
			this.m_openId=-1;
			...
			while (this.m_openCount > 0)
			{
				...
				{
					return this.getPath(p_startX, p_startY, currId).reverse();
				}
				...
				{
					//计算F和G值
					cost=this.m_movementCostList[currId] + ((note.x == currNoteX || note.y == currNoteY) ? COST_STRAIGHT : COST_DIAGONAL);
					score=cost + (Math.abs(p_endX - note.x) + Math.abs(p_endY - note.y)) * COST_STRAIGHT;
					if (this.isOpen(note.x, note.y)) //如果节点已在播放列表中
					{
						checkingId=this.m_noteMap[note.y][note.x][NOTE_ID];
		 ...
		 */
		private function getArounds(p_x:int, p_y:int):Vector.<Point>
		{
			...
			checkX=p_x + 1;
			checkY=p_y;
			var canRight:Boolean=this.m_map.isRoad(checkX, checkY);
			if (canRight && !this.isClosed(checkX, checkY))
			{
				arr.push(new Point(checkX,checkY));
			}
			//下
			checkX=p_x;
			checkY=p_y + 1;
			var canDown:Boolean=this.m_map.isRoad(checkX, checkY);
			...
			checkX=p_x - 1;
			checkY=p_y;
			var canLeft:Boolean=this.m_map.isRoad(checkX, checkY);
			...
			checkX=p_x;
			checkY=p_y - 1;
			var canUp:Boolean=this.m_map.isRoad(checkX, checkY);
			...
			checkX=p_x + 1;
			checkY=p_y + 1;
			canDiagonal=this.m_map.isRoad(checkX, checkY);
		 ...
		 */
		private function getPath(p_startX:int, p_startY:int, p_id:int):Vector.<Point>
		{
			var arr:Vector.<Point>=new Vector.<Point>();
			var noteX:int=this.m_xList[p_id];
			var noteY:int=this.m_yList[p_id];
