...
{
	import azura.banshee.editors.zforest.Zway;
	import azura.banshee.zebra.Zebra;
	import azura.banshee.zebra.editor.ztree.Ztree;
	...
	public class Zforest implements GsI
	{
		...
		public function loadZway(data:ZintBuffer):void{
			way=new Zway();
			way.fromBytes(data);
			zbaseScale=FastMath.pow2(base.zMax-way.base.zMax);
		}
		...
		public function getMc5List():Vector.<String>{
			...
			var mc5:String;
			for each(mc5 in land.getMc5List()){
				list.push(mc5);	
			}
			for each(mc5 in mask.getMc5List()){
				list.push(mc5);	
			}
			for each(var zt:Ztree in ztreeList){
				for each(mc5 in zt.zebra.getMc5List()){
					list.push(mc5);	
				}
			}
			return list;
